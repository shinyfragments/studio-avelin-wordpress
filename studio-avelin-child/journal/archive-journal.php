...
<main id="primary" class="sa-page sa-journal">
	...
	<section class="sa-journal-container sa-archive"><header class="sa-archive__head"><div><h2><?php echo esc_html( $is_filter ? 'Einträge' : 'Alle Einträge' ); ?></h2><p><?php echo esc_html( 'Die neuesten Notizen aus dem Studio.' ); ?></p></div><div class="sa-archive__tools"><nav aria-label="<?php echo esc_attr( 'Journal-Kategorien' ); ?>"><a class="<?php echo ! $is_filter ? 'is-active' : ''; ?>" href="<?php echo esc_url( get_post_type_archive_link( 'sa_journal' ) ); ?>"><?php echo esc_html( 'Alle' ); ?></a><?php foreach ( $categories as $category ) : ?><a class="<?php echo $is_filter && $term->term_id === $category->term_id ? 'is-active' : ''; ?>" href="<?php echo esc_url( get_term_link( $category ) ); ?>"><?php echo esc_html( $category->name ); ?></a><?php endforeach; ?></nav><label class="sa-journal-search"><span class="screen-reader-text"><?php echo esc_html( 'Einträge durchsuchen' ); ?></span><span aria-hidden="true">⌕</span><input type="search" data-journal-search placeholder="<?php echo esc_attr( 'Einträge und Tags durchsuchen …' ); ?>"></label></div></header>
