...
<main id="primary" class="sa-page sa-journal"><article class="sa-single"><div class="sa-journal-container sa-single__grid">
	...
	<div><p class="sa-journal-label"><?php esc_html_e( 'Share', 'studio-avelin-child' ); ?></p><button class="sa-copy-link" type="button" data-copy-link data-default-label="<?php esc_attr_e( 'Copy link', 'studio-avelin-child' ); ?>"><?php esc_html_e( 'Copy link', 'studio-avelin-child' ); ?></button></div>
	...
	<div class="sa-single__main"><header class="sa-single__header"><p class="sa-single__meta"><?php if ( $category ) : ?><a href="<?php echo esc_url( get_term_link( $category ) ); ?>"><?php echo esc_html( $category->name ); ?></a><span> · </span><?php endif; ?><time datetime="<?php echo esc_attr( get_the_date( 'c' ) ); ?>"><?php echo esc_html( strtoupper( get_the_date( 'F j, Y' ) ) ); ?></time><span> · </span><?php echo esc_html( sa_journal_reading_time() ); ?> MIN READ</p><h1><?php the_title(); ?></h1><span class="sa-journal-rule" aria-hidden="true"></span><?php if ( has_excerpt() ) : ?><p class="sa-single__lede"><?php echo esc_html( get_the_excerpt() ); ?></p><?php endif; ?><figure class="sa-single__cover"><?php get_template_part( 'journal/post-cover', null, array( 'size' => 'full' ) ); ?></figure></header>
