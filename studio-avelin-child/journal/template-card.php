...
<article <?php post_class( 'sa-journal-card' ); ?> data-journal-card data-search-text="<?php echo esc_attr( get_the_title() . ' ' . get_the_excerpt() . ' ' . implode( ' ', $category_names ) . ' ' . implode( ' ', wp_get_post_terms( get_the_ID(), 'sa_journal_tag', array( 'fields' => 'names' ) ) ) ); ?>">
	<a class="sa-journal-card__media" href="<?php the_permalink(); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Read %s', 'studio-avelin-child' ), get_the_title() ) ); ?>">
	...
	<div class="sa-journal-card__body">
		...
		<div class="sa-journal-card__foot"><span><?php echo esc_html( sa_journal_reading_time() ); ?> min</span><a href="<?php the_permalink(); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Read %s', 'studio-avelin-child' ), get_the_title() ) ); ?>">→</a></div>
	</div>
</article>
